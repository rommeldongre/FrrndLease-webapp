...
public class GrantLease extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			res1 = aoh1.getInfo(table, obj1); // goes to requests table -
												// edits()
			LOGGER.info(res1.getCode());
			if (res1.getIntCode() == e.FLS_SUCCESS) { // 56 is the success code
				LOGGER.info("Request for that item archived in requests table.");
				...
				res2 = aoh2.getInfo(table, obj2);// goes to store table -
													// edits()

				if (res2.getIntCode() == e.FLS_SUCCESS) {
					LOGGER.warning("Item entry deleted from store table");
					...
					res3 = aoh3.getInfo(table, obj3);// goes to items table -
														// editstat()

					if (res3.getIntCode() == e.FLS_SUCCESS) {
						LOGGER.warning("Item status updated to leased");
						...
						res4 = aoh4.getInfo(table, obj4);// goes to leases table
															// - add()
				...
				LOGGER.warning("Couldn't perform Grant Lease");
			}
