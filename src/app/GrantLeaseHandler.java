...
//				newE.send(rq.getReqUserId(), Notification_Type.FLS_MAIL_GRANT_LEASE_TO, rq);
				...
				event.createEvent(rq.getReqUserId(), rq.getUserId(), Event_Type.FLS_EVENT_NOTIFICATION, Notification_Type.FLS_MAIL_GRANT_LEASE_FROM, rq.getItemId(), "You have sucessfully leased an item to <a href=\"myapp.html#/myleasedoutitems\">" + rq.getReqUserId() + "</a> on Friend Lease ", rq);
				event.createEvent(rq.getUserId(), rq.getReqUserId(), Event_Type.FLS_EVENT_NOTIFICATION, Notification_Type.FLS_MAIL_GRANT_LEASE_TO, rq.getItemId(), "An item has been leased by <a href=\"myapp.html#/myleasedinitems\">" + rq.getUserId() + "</a> to you on Friend Lease ", rq);
			} catch (Exception e) {
				e.printStackTrace();
			}						
