...
public class GetLeaderBoardByXHandler extends Connect implements AppHandler {
	...
	public ResObj process(ReqObj req) throws Exception {
		...
		try {
			String GetItemSql = "SELECT tb1.request_item_id AS most_requested_item_id, tb2.item_name AS most_requested_item_name, tb3.user_full_name AS most_requested_item_user_name FROM `requests` tb1 INNER JOIN `items` tb2 ON tb1.request_item_id = tb2.item_id INNER JOIN `users` tb3 ON tb1.request_requser_id = tb3.user_id GROUP BY tb1.request_item_id ORDER BY COUNT(*) DESC LIMIT 3";
