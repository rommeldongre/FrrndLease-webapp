...
public class FlsSendMail {
	...
	public static void send(String userId, Fls_Enum fls_enum, Object obj, String... apiflag) throws Exception {
		...
		email.setAuthenticator(new DefaultAuthenticator("aniruddh@greylabs.org", "Windows@123"));
		email.setDebug(true);
		...
		case FLS_MAIL_REJECT_REQUEST_FROM:
			...
			email.setMsg("Request for item having id [" + dfrm.getItemId()
					+ "] has been removed as a lease might be granted. \n \n");
			break;

		case FLS_MAIL_REJECT_REQUEST_TO:
			RequestsModel dtrm = (RequestsModel) obj;
			email.setSubject("Request removed");
			email.setMsg("Request of item having id [" + dtrm.getItemId()
					+ "] has been removed by the owner as a lease might be granted. \n \n");
			break;
		...
		case FLS_MAIL_GRANT_LEASE_FROM:
			...
			email.setMsg(
					"You have sucessfully leased an item to [" + gflm.getReqUserId() + "] on Friend Lease - \n \n");
			break;
		...
		case FLS_MAIL_REJECT_LEASE_FROM:
			...
			email.setMsg("You have closed leased of item having id [" + rflm.getItemId() + "] and leasee ["
		...
		case FLS_MAIL_MAKE_REQUEST_FROM:
			...
			email.setMsg("You have sucessfully Requested an item having id [" + rfrm.getItemId()
					+ "] on Friend Lease - \n \n");
			break;
		...
		case FLS_MAIL_MAKE_REQUEST_TO:
			...
			email.setSubject("Item Requested");
			email.setMsg("Your Item [" + irtm.getTitle() + "] having id [" + irtm.getId()
					+ "] has been requested on Friend Lease - \n \n");
			break;
